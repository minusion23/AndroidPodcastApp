...
public class QueryUtilities {
    ...
    private static List<PodcastSearchItem> extractFeatureFromJson(String jsonResponse) {
        ...
        try {
            ...
            JSONObject feedJson = baseJSONResponse.getJSONObject("feed");
            JSONArray  resultsArray = feedJson.getJSONArray("results");

            Log.v("baseJSON", String.valueOf(baseJSONResponse.length()));
            for (int i = 0; i < resultsArray.length(); i++) {
                ...
                id = currentResult.optString("id");
                name = currentResult.optString("name");
                artwork= currentResult.optString("artworkUrl100");
                feedUrl = currentResult.optString("feedUrl","No data");
        ...
        Log.v("recipeItems", podcastItems.toString());
        return podcastItems;
    }
